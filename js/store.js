...
(function (window) {
	...
	Store.prototype.find = function (query, callback) {
		...
		fetch(
			`https://latest.datasette.io/ephemeral/${this._dbName}.json?_shape=array&_sort_desc=id&completed=${query.completed && '1' || '0'}`, {
					mode: "cors",
					headers: {
						Authorization: `Bearer ${TOKEN}`,
						"Content-Type": "application/json"
	...
	Store.prototype.save = function (updateData, callback, id) {
		...
		if (id) {
			...
			fetch(
				`https://latest.datasette.io/ephemeral/${self._dbName}/-/insert`, {
						method: "POST",
						mode: "cors",
						headers: {
							Authorization: `Bearer ${TOKEN}`,
